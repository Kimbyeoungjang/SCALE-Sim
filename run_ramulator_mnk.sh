...
python3 -m scalesim.scale -c ./configs/google.cfg -t "${topology}" -i gemm --save-trace > "${model}_${buf}_orig_out"
...
python3 -m scalesim.scale -c ./configs/google_ramulator.cfg -t "${topology}" -i gemm --no-save-trace > "${model}_${buf}_stall_out"
